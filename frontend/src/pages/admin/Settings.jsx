...
import { Save, Settings as SettingsIcon, CreditCard, Mail, ShieldCheck, RefreshCw, AlertTriangle } from "lucide-react";
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        setStripeActive(Boolean(data.payments?.stripeActive));
        ...
        const wl = Array.isArray(data.security?.ipWhitelist)
          ? data.security.ipWhitelist.join(", ")
          : typeof data.security?.ipWhitelist === "string"
            ? data.security.ipWhitelist
            : "";
        setIpWhitelist(wl);
      }
    } catch (_err) {
      console.error("Error fetching settings:", _err);
          ...
          <div className="max-w-4xl mx-auto space-y-8 pb-20">
            ...
            {error && (
              <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-2xl flex items-center justify-between text-xs font-semibold">
                ...
                <button onClick={fetchSettings} className="underline hover:text-red-900">Retry</button>
              </div>
            )}

            {/* Main Tabs Container Wrapped in Error Boundary */}
            <AdminErrorBoundary title="Unable to load system settings dashboard">
              <div className="bg-white border rounded-[2rem] shadow-xs overflow-hidden flex flex-col md:flex-row min-h-[500px]">
                ...
                <div className="flex-1 p-6 sm:p-8">
                  {loading ? (
                    <p className="text-center text-slate-400 font-semibold py-12">Loading settings configs...</p>
                  ) : (
