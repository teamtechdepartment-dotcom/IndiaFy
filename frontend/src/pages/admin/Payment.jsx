...
import { Download, IndianRupee, RotateCcw, Wallet, Settings, Percent, AlertTriangle, RefreshCw } from "lucide-react";
...
export default function Payments() {
  ...
  const fetchFinancials = async () => {
    ...
    try {
      ...
      console.error("Error loading financials:", _err);
  ...
  const handleUpdateCommission = async (e) => {
    ...
    try {
      await axiosInstance.put("/admin/management/settings", {
        commissions: {
          globalRate: parseFloat(globalRate) || 5.0,
          categoryRates: {
            [categoryName]: parseFloat(categoryRate) || 3.0
          }
        }
      });
  ...
  const handleExport = () => {
    try {
      const safeTxs = Array.isArray(financials?.transactions) ? financials.transactions : [];
      if (safeTxs.length === 0) {
        toast.info("No transaction data available for export");
        return;
      }
      exportToCSV(safeTxs, "payments-ledger.csv");
    } catch (err) {
          ...
          <div className="max-w-7xl mx-auto space-y-8 pb-20">
            ...
            {error && (
              <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-2xl flex items-center justify-between text-xs font-semibold">
                ...
                <button onClick={fetchFinancials} className="underline hover:text-red-900">Retry</button>
              </div>
            )}

            {/* Stats Cards Wrapped in Error Boundary */}
            <AdminErrorBoundary title="Unable to load payment metrics">
                ...
                {loading ? (
                  ...
                  <>
                    ...
                    <StatsCard
                      ...
                      value={`₹${netCommission.toLocaleString()}`}
                      accent="green"
            ...
            <AdminErrorBoundary title="Unable to load financial ledger">
                        ...
                        {loading ? (
                          [1, 2, 3].map(idx => (
                            <tr key={idx} className="animate-pulse">
                              ...
                              <td className="py-4 px-4 text-center"><div className="w-12 h-4 bg-slate-200 rounded mx-auto" /></td>
                              <td className="py-4 px-4 text-center"><div className="w-16 h-4 bg-slate-200 rounded mx-auto" /></td>
                              <td className="py-4 px-4 text-right"><div className="w-20 h-3 bg-slate-200 rounded ml-auto" /></td>
                            </tr>
                          ))
                        ) : txs.length === 0 ? (
                          <>
                          ...
                          txs.map((tx) => {
                            ...
                            const dateStr = tDate && !isNaN(tDate.getTime()) ? tDate.toLocaleDateString() : "Today";
                            return (
                              <LedgerRow 
...
function LedgerRow({ id = "TX-000", amount = 0, method = "UPI", status = "Success", date = "Today" }) {
  return (
    <tr className="hover:bg-slate-50/50 transition-colors">
      <td className="py-4 px-4 font-bold text-[#D4AF37]">{id}</td>
      <td className="py-4 px-4 font-black text-gray-900">₹{Number(amount).toLocaleString()}</td>
