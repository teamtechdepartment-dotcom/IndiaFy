...
export default function BrandLandingPage() {
  ...
  useEffect(() => {
    ...
    // Fetch brand SEO data
    axiosInstance.get(`/content/brand/${brandSlug}`)
      .then(res => {
        const data = res.data?.data || res.data;
        setSeoData(data);
        
        // Fetch popular products for this brand
        return axiosInstance.get(`/products`, { params: { brand: data.brand } });
      })
      .then(res => {
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col justify-between font-sans">
        ...
        <div className="pt-[130px] md:pt-[160px] pb-20 text-center max-w-md mx-auto px-4">
          ...
          <p className="text-zinc-500 mb-6">The brand you are looking for does not exist or has insufficient products to display.</p>
    ...
    <div className="min-h-screen bg-slate-50 font-sans selection:bg-emerald-200 selection:text-emerald-900">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 pt-[130px] lg:pt-[140px] pb-16">
        <div className="flex items-center gap-2 text-xs font-medium text-slate-500 mb-4">
          ...
          <span className="text-slate-900 font-bold">{seoData.brand}</span>
        </div>

        <div className="mb-6 max-w-4xl">
          <h1 className="text-3xl font-bold text-slate-900 mb-2">{seoData.brand} Products</h1>
          <p className="text-sm text-slate-600 leading-relaxed">{seoData.intro}</p>
          {seoData.relatedCategories && seoData.relatedCategories.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-2 text-xs">
              <span className="font-semibold text-slate-700">Shop by Category:</span>
              {seoData.relatedCategories.map(cat => (
                <Link 
                  key={cat} 
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-4">
          <div className="mb-4">
             <h2 className="text-lg font-bold text-slate-800">Popular Products ({seoData.activeProductCount})</h2>
          </div>
