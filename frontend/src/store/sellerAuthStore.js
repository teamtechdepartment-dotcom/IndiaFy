  ...
  persist(
    (set, get) => ({
      ...
      fetchMe: async (role, retries = 2) => {
        ...
        // If no token is stored, the user is definitely not logged in — skip the network call
        // entirely to avoid the 401 → refresh → 401 cascade on page load.
        if (!state.token && !state.isAuthenticated) {
          return;
        }

        // If the persisted session is past its expiry window, clear immediately
