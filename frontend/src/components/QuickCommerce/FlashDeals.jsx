...
export default function FlashDeals({ onAdd, isLoading }) {
    ...
    axiosInstance.get("/products/featured")
      .then(res => {
        ...
        setProducts(raw.map(p => {
          ...
          const mrp = p.attribute?.mrpPrice ?? p.price ?? price;
          ...
          return {
            ...
            mrp: mrp,
            discount: discount,
            img: p.productImage?.[0] || "https://placehold.co/400"
          };
        }));
      })
      .catch(err => {
  ...
  if (isLoading || isLocalLoading) return null;
