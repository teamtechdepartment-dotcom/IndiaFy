...
const Signup = async (req, res) => {
  try {
    ...
    if (!passwordRegex.test(password)) {
      console.log(
        "Seller Signup failed: Password complexity requirements not met.",
      );
      return res
        .status(400)
        .json(
    ...
    if (!sellerDetails) {
      return res
        .status(400)
        .json(new ApiError(400, "New Seller registration failed"));
    }
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, tokenData, "New Seller registration Successful"),
      );
  } catch (err) {
    return res
      .status(500)
...
const Login = async (req, res) => {
  try {
    ...
    console.log(
      "Seller Password verification result:",
