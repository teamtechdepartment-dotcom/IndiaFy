...
const Signup = async (req, res) => {
  try {
    ...
    if (!passwordRegex.test(password)) {
      console.log(
        "Signup failed: Password does not meet complexity requirements.",
      );
      return res
        .status(400)
        .json(
    ...
    if (!customerDetails) {
      return res
        .status(400)
        .json(new ApiError(400, "New Customer registration failed"));
    }
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, tokenData, "New Customer registration Successful"),
      );
  } catch (err) {
    return res
      .status(500)
