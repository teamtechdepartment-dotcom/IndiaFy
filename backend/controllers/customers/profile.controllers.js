...
export const deleteAccount = async (req, res) => {
    try {
        ...
        if (!password) {
            return res.status(400).json(new ApiError(400, "Password is required to confirm account deletion"));
        }

        // Fetch auth account to verify password
        let userData = null;
        if (role === "Seller") {
            const SellerModel = (await import("../../models/sellers/auth.model.js")).default;
            userData = await SellerModel.findById(userId);
        ...
        const { passwordDecryption } = await import("../../utils/bcrypt.js");
        const isMatch = await passwordDecryption(password, userData.password);
        if (!isMatch) {
            return res.status(401).json(new ApiError(401, "Incorrect password. Account deletion aborted."));
        }

        // Delete profile
        await CustomerProfile.findOneAndDelete({ customerId: userId });

        // Delete auth account from DB
        if (role === "Seller") {
            const SellerModel = (await import("../../models/sellers/auth.model.js")).default;
        ...
        // Clear role-specific cookies safely
        ...
        res.clearCookie(`${rolePrefix}AccessToken`, clearOptions);
        res.clearCookie(`${rolePrefix}RefreshToken`, clearOptions);
